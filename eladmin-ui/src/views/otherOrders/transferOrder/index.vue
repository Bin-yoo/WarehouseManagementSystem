  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="showDisable ? '查看单据' : crud.status.title" width="1000px" @closed="closeDialog">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          <div class="order_info">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="调出仓库" prop="sourceId">
                  <el-select
                    ...
                    :disabled="showDisable"
                    clearable
                    @change="changeOutWarehouse"
                  >
                    <el-option
                      v-for="item in dialogOutWhSelect"
                      :key="item.whId"
                      :label="item.whName"
                      :value="item.whId"
                    />
              ...
              <el-col :span="8">
                <el-form-item label="调入仓库" prop="whId">
                  <el-select
                    v-model="form.whId"
                    placeholder="请选择调入仓库"
                    style="width: 90%"
        ...
        <GoodChooseBoard :inner-visible="innerVisible" :wh-id="form.sourceId" stock-out @cancel="cancelPickUp" @pickup="pickup" />
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column prop="date" label="调拨日期" width="90px">
          <template slot-scope="scope">
            {{ scope.row.date | formatDate("") }}
          </template>
        </el-table-column>
        <el-table-column prop="whName" label="调出仓库" />
        <el-table-column prop="sourceName" label="调入仓库" width="120px" />
        ...
        <el-table-column v-if="checkPer(['admin','transferOrders:edit','transferOrders:del','transferOrders:approve','transferOrders:reApprove','transferOrders:print'])" fixed="right" label="操作" width="295px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
export default {
  ...
  cruds() {
    return CRUD({ title: '仓库调拨单', url: 'api/transferOrders', idField: 'id', sort: 'id,desc', crudMethod: { ...TransferOrders }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'transferOrders:add'],
        edit: ['admin', 'transferOrders:edit'],
        del: ['admin', 'transferOrders:del'],
        approve: ['admin', 'transferOrders:approve'],
        reApprove: ['admin', 'transferOrders:reApprove'],
        print: ['admin', 'transferOrders:print']
      },
      rules: {
        managerId: [
        ...
        whId: [
          { required: true, message: '必须选择调出仓库', trigger: 'blur' }
        ],
        sourceId: [
          { required: true, message: '必须选择调入仓库', trigger: 'blur' }
        ],
        originOrderNo: [
  ...
  methods: {
    ...
    [CRUD.HOOK.afterToCU](crud, form) {
      ...
      if (form.id == null) {
        ...
        this.getOrderGoodList(form.id, form.sourceId)
      }
    },
    // 提交前的验证
    ...
    getManagerSelect() {
      TransferOrders.getManagerSelect().then(res => {
        this.managerSelect = res
      })
    },
    getOrderNo() {
      TransferOrders.getOrderNo().then(res => {
    ...
    changeOutWarehouse(val) {
      ...
      if ((val !== null && val !== '') && this.form.whId === val) {
        this.form.sourceId = null
        this.$notify({
          title: '选择的调出仓库不能跟调入仓库相同!',
          type: 'warning',
          duration: 2500
        })
        return
      }
      const obj = this.dialogOutWhSelect.find((item) => {
        return item.whId === val
      })
      this.form.sourceName = obj.whName
    },
    changeInWarehouse(val) {
      if ((val === null || val === '')) {
        return
      }
      if ((val !== null && val !== '') && this.form.sourceId === val) {
        this.form.whId = null
        this.$notify({
          title: '选择的调入仓库不能跟调出仓库相同!',
          type: 'warning',
          duration: 2500
        })
        return
    ...
    pickGood() {
      if (this.form.sourceId == null || this.form.sourceId === '') {
        this.$notify({
          title: '请先选择调出仓库!',
          type: 'warning',
          duration: 2500
    ...
    getOrderGoodList(id, whId) {
      ...
      TransferOrders.getOrderGoodList(params).then(res => {
    ...
    toApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return TransferOrders.approveOrders(ids).then(() => {
          if (processAll) {
    ...
    toReApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return TransferOrders.reApproveOrders(ids).then(() => {
          if (processAll) {
