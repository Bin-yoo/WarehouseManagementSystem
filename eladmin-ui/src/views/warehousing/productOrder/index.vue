  ...
  <div class="app-container">
    ...
    <div class="head-container">
        ...
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          <div class="order_info">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="生产部门" prop="sourceId">
                  <treeselect
                    v-model="form.sourceId"
                    :options="depts"
                    :load-options="loadDepts"
                    :disabled="showDisable"
                    :class="{ dialog_tree_select : true, dialog_tree_select_disable : showDisable }"
                    style="width: 90%"
                    placeholder="选择生产部门"
                    @select="changeDept"
                  />
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column prop="sourceName" label="生产部门" width="120px" />
        <el-table-column prop="originOrderNo" label="原始单号" />
        <el-table-column prop="status" label="生产单状态">
          <template slot-scope="scope">
        ...
        <el-table-column v-if="checkPer(['admin','productOrders:edit','productOrders:del','productOrders:approve','productOrders:reApprove','productOrders:print'])" fixed="right" label="操作" width="295px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
            ...
            >
              ...
              <el-button slot="right" size="mini" icon="el-icon-printer" v-permission="permission.print" @click="toPrint(scope.row)" />
            </udOperation>
          </template>
        </el-table-column>
      </el-table>
...
import OrderPrinting from '@/components/OrderPrintingTemplate/ProductOrderTemplate'
...
export default {
  name: 'ProductOrders',
  components: { pagination, crudOperation, rrOperation, udOperation, DateRangePicker, GoodChooseBoard, OrderPrinting, Treeselect },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  cruds() {
    return CRUD({ title: '生产入库单', url: 'api/productOrders', idField: 'id', sort: 'id,desc', crudMethod: { ...ProductOrders }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'productOrders:add'],
        edit: ['admin', 'productOrders:edit'],
        del: ['admin', 'productOrders:del'],
        approve: ['admin', 'productOrders:approve'],
        reApprove: ['admin', 'productOrders:reApprove'],
        print: ['admin', 'productOrders:print']
      },
      rules: {
        managerId: [
        ...
        sourceId: [
          { required: true, message: '必须选择生产部门', trigger: 'blur' }
        ],
        originOrderNo: [
  ...
  methods: {
    ...
    [CRUD.HOOK.afterToCU](crud, form) {
      ...
      if (form.id == null) {
        ...
        this.getSupDepts(parseInt(form.sourceId))
      }
    },
    // 提交前的验证
    ...
    getManagerSelect() {
      ProductOrders.getManagerSelect().then(res => {
        this.managerSelect = res
      })
    },
    getOrderNo() {
      ProductOrders.getOrderNo().then(res => {
        this.form.orderNo = res
      })
    },
    changeDept(node, instanceId) {
      this.form.sourceName = node.label
    },
    changeWarehouse(val) {
    ...
    getOrderGoodList(id) {
      ...
      ProductOrders.getOrderGoodList(id).then(res => {
    ...
    toApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return ProductOrders.approveOrders(ids).then(() => {
          if (processAll) {
    ...
    toReApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return ProductOrders.reApproveOrders(ids).then(() => {
          if (processAll) {
