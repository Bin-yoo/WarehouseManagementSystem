  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="checkPer(['admin','sellRefundOrders:edit','sellRefundOrders:del','sellRefundOrders:approve','sellRefundOrders:reApprove','sellRefundOrders:print'])" fixed="right" label="操作" width="295px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
<script>
import SellRefundOrders from '@/api/sellRefundOrders.js'
...
export default {
  ...
  data() {
    return {
      permission: {
        add: ['admin', 'sellRefundOrders:add'],
        edit: ['admin', 'sellRefundOrders:edit'],
        del: ['admin', 'sellRefundOrders:del'],
        approve: ['admin', 'sellRefundOrders:approve'],
        reApprove: ['admin', 'sellRefundOrders:reApprove'],
        print: ['admin', 'sellRefundOrders:print']
      },
      rules: {
        managerId: [
        ...
        whId: [
          { required: true, message: '必须选择退入仓库', trigger: 'blur' }
        ],
        sourceId: [
  ...
  methods: {
    ...
    getCustomerSelect() {
      SellRefundOrders.getCustomerSelect().then(res => {
    ...
    getManagerSelect() {
      SellRefundOrders.getManagerSelect().then(res => {
        this.managerSelect = res
      })
    },
    getOrderNo() {
      SellRefundOrders.getOrderNo().then(res => {
    ...
    getOrderGoodList(id) {
      ...
      SellRefundOrders.getOrderGoodList(id).then(res => {
    ...
    toApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return SellRefundOrders.approveOrders(ids).then(() => {
          if (processAll) {
    ...
    toReApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return SellRefundOrders.reApproveOrders(ids).then(() => {
          if (processAll) {
