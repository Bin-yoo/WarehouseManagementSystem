  ...
  <div class="app-container">
    ...
    <div class="head-container">
        ...
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          <div class="order_info">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="退货仓库" prop="whId">
                  <el-select
                    v-model="form.whId"
                    placeholder="请选择退货仓库"
                    style="width: 90%"
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="checkPer(['admin','purchaseRefundOrders:edit','purchaseRefundOrders:del','purchaseRefundOrders:approve','purchaseRefundOrders:reApprove','purchaseRefundOrders:print'])" fixed="right" label="操作" width="295px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
import OrderPrinting from '@/components/OrderPrintingTemplate/PurchaseRefundOrderTemplate'
import PurchaseOrderSelectBoard from '@/components/PurchaseOrderSelectBoard'
...
export default {
  name: 'PurchaseRefundOrders',
  components: { pagination, crudOperation, rrOperation, udOperation, DateRangePicker, GoodChooseBoard, OrderPrinting, PurchaseOrderSelectBoard },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  cruds() {
    return CRUD({ title: '采购退货单', url: 'api/purchaseRefundOrders', idField: 'id', sort: 'id,desc', crudMethod: { ...PurchaseRefundOrders }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'purchaseRefundOrders:add'],
        edit: ['admin', 'purchaseRefundOrders:edit'],
        del: ['admin', 'purchaseRefundOrders:del'],
        approve: ['admin', 'purchaseRefundOrders:approve'],
        reApprove: ['admin', 'purchaseRefundOrders:reApprove'],
        print: ['admin', 'purchaseRefundOrders:print']
      },
      rules: {
        managerId: [
  ...
  methods: {
    ...
    getSupplierSelect() {
      PurchaseRefundOrders.getSupplierSelect().then(res => {
    ...
    getManagerSelect() {
      PurchaseRefundOrders.getManagerSelect().then(res => {
        this.managerSelect = res
      })
    },
    getOrderNo() {
      PurchaseRefundOrders.getOrderNo().then(res => {
    ...
    toApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return PurchaseRefundOrders.approveOrders(ids).then(() => {
          if (processAll) {
    ...
    toReApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return PurchaseRefundOrders.reApproveOrders(ids).then(() => {
          if (processAll) {
