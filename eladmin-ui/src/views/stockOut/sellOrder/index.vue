        ...
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          <div class="order_info">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="出货仓库" prop="whId">
                  <el-select
                    v-model="form.whId"
                    placeholder="请选择出货仓库"
                    style="width: 90%"
...
<script>
import SellOrders from '@/api/sellOrders.js'
...
export default {
  ...
  methods: {
    ...
    getCustomerSelect() {
      SellOrders.getCustomerSelect().then(res => {
    ...
    getManagerSelect() {
      SellOrders.getManagerSelect().then(res => {
        this.managerSelect = res
      })
    },
    getOrderNo() {
      SellOrders.getOrderNo().then(res => {
    ...
    toApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return SellOrders.approveOrders(ids).then(() => {
          if (processAll) {
    ...
    toReApprove(datas) {
      ...
      this.$confirm(confirm, '提示', {
        ...
        return SellOrders.reApproveOrders(ids).then(() => {
          if (processAll) {
