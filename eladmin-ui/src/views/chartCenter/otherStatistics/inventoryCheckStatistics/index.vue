<template>
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <!--表单组件-->
      <el-dialog :close-on-click-modal="false" :visible.sync="dialogShow" :title="crud.status.title" width="1250px" @closed="dialogClose">
        <el-row>
          <el-table v-loading="detailDataLoading" :data="detailData" size="small" style="width: 100%;">
            ...
            <!-- <el-table-column prop="sourceName" label="调出仓库" /> -->
            <el-table-column prop="whName" label="盘点仓库" />
            <el-table-column prop="date" label="盘点日期" width="85" >
              <template slot-scope="scope">
            ...
            <el-table-column prop="plGoodNum" label="盈亏数量">
              <template slot-scope="scope">
                <span>¥&nbsp;{{ scope.row.plGoodNum }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="plPrice" label="盈亏金额">
              <template slot-scope="scope">
                <span>¥&nbsp;{{ scope.row.plPrice }}</span>
              </template>
            </el-table-column>
          </el-table>
        </el-row>
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="checkPer(['admin','supplierOfferStatistics:list'])" fixed="right" label="操作" align="center">
          <template slot-scope="scope">
...
<script>
import InventoryCheckStatistics from '@/api/inventoryCheckStatistics.js'
...
export default {
  name: 'InventoryCheckStatistics',
  components: { pagination, crudOperation, rrOperation, udOperation },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  cruds() {
    return CRUD({ title: '查看单据详情', url: 'api/inventoryCheckStatistics', idField: 'id', sort: 'goodId,desc', crudMethod: { ...InventoryCheckStatistics }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'inventoryCheckStatistics:add'],
        edit: ['admin', 'inventoryCheckStatistics:edit'],
        del: ['admin', 'inventoryCheckStatistics:del']
      },
      dialogShow: false,
      detailData: [],
  ...
  methods: {
    ...
    showDetail(goodId) {
      ...
      InventoryCheckStatistics.getDetail(params).then(res => {
