  ...
  <div class="app-container">
    ...
    <div class="head-container">
          ...
          <el-table v-loading="detailDataLoading" :data="detailData" size="small" style="width: 100%;">
            <el-table-column prop="orderType" label="单据类型">
              <template slot-scope="scope">
                <span v-if="scope.row.orderType === 6">调拨出库单</span>
                <span v-if="scope.row.orderType === 8">调拨入库单</span>
              </template>
            </el-table-column>
            <el-table-column prop="orderNo" label="单号" width="150" />
            <el-table-column prop="sourceName" label="调出仓库" />
            <el-table-column prop="whName" label="调入仓库" />
            <el-table-column prop="date" label="调拨日期" width="85" >
              <template slot-scope="scope">
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column prop="transferCount" label="调拨数量" />
        <el-table-column prop="transferAmount" label="调拨金额" >
          <template slot-scope="scope">
...
<script>
import TransferStatistics from '@/api/transferStatistics.js'
...
export default {
  ...
  cruds() {
    return CRUD({ title: '查看单据详情', url: 'api/transferStatistics', idField: 'id', sort: 'goodId,desc', crudMethod: { ...TransferStatistics }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'transferStatistics:add'],
        edit: ['admin', 'transferStatistics:edit'],
        del: ['admin', 'transferStatistics:del']
      },
      dialogShow: false,
      detailData: [],
  ...
  methods: {
    ...
    showDetail(goodId) {
      ...
      TransferStatistics.getDetail(params).then(res => {
