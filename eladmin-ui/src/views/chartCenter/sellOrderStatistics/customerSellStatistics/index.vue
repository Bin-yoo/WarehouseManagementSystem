...
<script>
import CustomerSellStatistics from '@/api/customerSellStatistics.js'
...
export default {
  name: 'CustomerSellStatistics',
  components: { pagination, crudOperation, rrOperation, udOperation, GoodChooseBoard },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  cruds() {
    return CRUD({ title: '查看单据详情', url: 'api/customerSellStatistics', idField: 'id', sort: 'a.id,desc', crudMethod: { ...CustomerSellStatistics }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'customerSellStatistics:add'],
        edit: ['admin', 'customerSellStatistics:edit'],
        del: ['admin', 'customerSellStatistics:del']
      },
      dialogShow: false,
      detailData: [],
  ...
  methods: {
    ...
    showDetail(sourceId) {
      ...
      CustomerSellStatistics.getDetail(params).then(res => {
