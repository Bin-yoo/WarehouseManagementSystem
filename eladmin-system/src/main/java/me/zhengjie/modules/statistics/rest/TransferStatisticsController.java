...
@Api(tags = "货品调拨汇总")
@RequestMapping("/api/transferStatistics")
public class TransferStatisticsController {
    ...
    @PreAuthorize("@el.check('transferStatistics:list')")
    public ResponseEntity query(ProductStatisticsQueryParam query, Pageable pageable){
        return new ResponseEntity<>(transferStatisticsService.queryAll(query, pageable),HttpStatus.OK);
    }
    ...
    public ResponseEntity getDetail(Long goodId, Pageable pageable){
        ...
        return new ResponseEntity<>(transferStatisticsService.getDetail(goodId, pageable),HttpStatus.OK);
    }
    ...
    public void download(HttpServletResponse response, TbOrdersQueryParam query) throws IOException {
        transferStatisticsService.download(transferStatisticsService.queryAll(query), response);
    }*/
